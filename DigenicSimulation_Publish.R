...
for (i in 1:10) {
  ...
  for (g in 1:120){
    ...
    if (g >= 1){ 
      ...
      NewProd_Females1 <- NewProdPop %>% filter(genoA == "a/a" & genoB == "b/b" &  sex == "F") %>% top_n(800, merit) ### higher retention so that there are enough passed down for BW females
      ...
      NewProd_Males1 <- NewProdPop %>% filter(genoA == "a/a" & genoB == "b/b" &  sex == "F") %>% top_n(50, merit) ### higher retention so that there are enough passed down for BW females
  ...
  OutputData_SPF <- function (popdata, label){
    ...
    MeritAve <- function (popdata) {popdata %>% filter(gen > 0) %>% group_by(gen) %>% dplyr::summarise (MeanMerit = mean(merit),.groups = "drop") }
  ...
  write.csv(OutputFile, paste0("DGt_MI_Acc1_0.2Mos_",i,".csv")) 
